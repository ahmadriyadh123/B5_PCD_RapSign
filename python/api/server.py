...
def preprocess_to_contour(image_bytes: bytes) -> tuple[np.ndarray, bytes]:
    """
    ...
    Returns:
        contour_image (np.ndarray): gambar hasil kontur (grayscale)
...
async def verify_signature(
    ...
    try:
        ...
        contour_image, contour_bytes = preprocess_to_contour(image_bytes)
        feature_ready = prepare_feature_ready(contour_image)
...
async def preview_contour(
    ...
    try:
        ...
        _, contour_bytes = preprocess_to_contour(image_bytes)
        contour_b64 = base64.b64encode(contour_bytes).decode("utf-8")
